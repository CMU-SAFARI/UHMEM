...
namespace MemMap
{
    public class DecoupledWBFullRDEmptyIdleRDBusyServeN : MemWBMode
    {
        ...
        public DecoupledWBFullRDEmptyIdleRDBusyServeN(MemCtrl[] mctrls)
            : base(mctrls)
        {
        ...
        private bool is_readq_busy(uint cid)
        {
            if (!rmpkis_valid)
                return false;
        ...
        public override void tick(uint cid)
        {
            ...
            for (uint i = 0; i < cmax; i++) {
                ...
                if (!is_writeq_empty(i) && !is_readq_busy(i) && serve_cnt[i] < serve_max)
                    continue;
            ...
            for (uint i = 0; i < cmax; i++) {
                ...
                if (!is_writeq_full(i) && !is_readq_empty(i) && !is_readq_idle(i))
                    continue;
