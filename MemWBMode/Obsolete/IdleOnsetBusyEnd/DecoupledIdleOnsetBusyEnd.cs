...
namespace MemMap
{
    public class DecoupledIdleOnsetBusyEnd : MemWBMode
    {
        ...
        public DecoupledIdleOnsetBusyEnd(MemCtrl[] mctrls)
            : base(mctrls)
        {
        ...
        public override void tick(uint cid)
        {
            ...
            for (uint i = 0; i < cmax; i++) {
                ...
                wb_mode_cycles[i]++;
                if (wb_mode_cycles[i] < min_wb_window)
                    continue;

                if (wb_mode_cycles[i] < max_wb_window) {
                    if (is_busy(i)) {
                        wb_mode[i] = false;
                    }
                    continue;
